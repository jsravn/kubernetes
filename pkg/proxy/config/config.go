...
type PodHandler interface {
	...
	// OnPodDelete is called whever deletion of an existing pod
	// object is observed.
...
func (c *PodConfig) Run(stopCh <-chan struct{}) {
	...
	glog.Info("Starting pod config controller")
	defer glog.Info("Shutting down pod config controller")

	if !controller.WaitForCacheSync("pod config", stopCh, c.listerSynced) {
		return
	}
...
func (c *PodConfig) handlePodService(obj interface{}) {
	pod, ok := obj.(*api.Pod)
	if !ok {
		tombstone, ok := obj.(cache.DeletedFinalStateUnknown)
		if !ok {
